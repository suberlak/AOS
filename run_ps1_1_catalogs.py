...
all_fields = gt_dict.keys()
fields = ['high']
instruments = ['comCam'] # or ['comCam', 'lsstCam']
...
def getStarCatalog(connection, ra, dec, field_name, instrument='comCam', 
                   passband="rmeanpsfmag", save_file=True,
...
for instrument in instruments:
    ...
    for field_name in fields:
        ...
        print(ra,dec,field_name)

        # query database for sources within a given radius,
        # store as ASCII file and return as a pandas dataframe
        pandaCat = getStarCatalog(connection, ra, dec, field_name,
                                  instrument, output_dir = root_dir)

        # add to the dictionary for fast access 
        catalogs[instrument][field_name] = pandaCat
...
def writePhosimInstFile(pandaCat, ra, dec,  phosimFile="stars.cat", passband='r', outDir="./", camconfig=3, opd=False,
                       exposure=.25, obsid=9006002, position='defocal'):
...
cmd_file = writePhosimCmdFile('/project/scichris/aos/ps1_phosim/', 
                              file_name='noBkgnd.cmd', 
                              noBackground=True)
...
for instrument in instruments:
    for field_name in fields:
        for position in ['focal','defocal']
            ...
            pandaCatalog = catalogs[instrument][field_name]
            
            inst_file = writePhosimInstFile(pandaCatalog, ra, dec,  
                                phosimFile=f"stars_{instrument}_PS1_{field_name}_{position}.inst", 
                                passband='r', outDir=path, exposure=0.25, 
                                obsid=9006002, position=position)
